...
export async function startup() {
  try {
    ...
    await app.listen({
      ...
      host: ConfigService.app.host,
    });
    
    app.log.info(`Server listening on http://${ConfigService.app.host}:${ConfigService.app.port}`);
    app.log.info(`Environment: ${ConfigService.app.env}`);
