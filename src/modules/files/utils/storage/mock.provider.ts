...
export class MockStorageProvider implements StorageProvider {
  ...
  verifyUrl(
    ...
    const boundLength = extras.len;
    if (presented.sizeBytes !== undefined && boundLength !== undefined) {
      if (presented.sizeBytes !== Number(boundLength)) return { ok: false, reason: 'wrong-length' };
    }
    return { ok: true, key };
  }
