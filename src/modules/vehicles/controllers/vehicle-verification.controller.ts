...
import { VehicleVerificationService } from '../services/vehicle-verification.service.js';
import { reviewVehicleSchema } from '../schemas/vehicle.schemas.js';
import { toVehicleView } from './vehicle.controller.js';

/// Admin-only. Route-level `authorize({ roles: ['admin'] })` is the gate; these
/// handlers assume it has already run, exactly as the driver review handlers do.
export class VehicleVerificationController {
  ...
  async reviewDocument(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    req.log.info(
      ...
      '[vehicles] document review decision recorded',
    );
  ...
  async reviewVehicle(req: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    req.log.warn(
      ...
      '[vehicles] verification decision recorded',
    );
