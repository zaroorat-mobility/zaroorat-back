...
## 53. Remediation — 2026-09-01

The audit above was read-only. This section records what was changed afterwards,
on explicit instruction, and how each change was verified. Findings are amended,
not rewritten: the original text stands.

### Closed

| Finding       | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Evidence                                                                                                                                                                                                                                     |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Q-1 / Q-2** | The `12400 m / 1860 s` constant and its four-condition trigger are gone from all three providers. One shared helper, `location/utils/offline-route.ts`, gated on `APP_ENV`/`NODE_ENV === 'test'` alone and derived from the coordinates. The `apiKey.startsWith('test_')` and `'mock_'` clauses — the production-reachable ones — are deleted.                                                                                                                                                                                                                                                     | Quote distances vary: 2 km → ₹60.78, 20 km → ₹279.33, cross-city → ₹282.46. Same point now returns `400 TRIP_HAS_NO_DISTANCE`, so the zero-distance guard is reachable again.                                                                |
| **Q-3**       | Follows from Q-1: no offline route can be produced outside a test environment, so `distanceSource` can no longer name a provider that did not supply the value.                                                                                                                                                                                                                                                                                                                                                                                                                                    | grep: no residual constant or credential-shaped gate in `src/`.                                                                                                                                                                              |
| **Q-4**       | `getCategorySettings` decrypts every secret it returns, so caching its output put the provider API keys in Redis in clear text for an hour. They are re-encrypted before the cache write; `decryptSecret` on the read side is idempotent and needed no change.                                                                                                                                                                                                                                                                                                                                     | `map-provider.service.ts` cache-write path.                                                                                                                                                                                                  |
| **P-2**       | `@socket.io/redis-adapter` installed and attached, replacing the `assertAdapterSupported()` throw. Two dedicated connections, released on `close()`.                                                                                                                                                                                                                                                                                                                                                                                                                                               | Two instances, ports 3001/3002: a socket on A received a `ride.requested` emitted by B's outbox relay. 6/6.                                                                                                                                  |
| **P-3**       | PgBouncer added to `compose.prod.yml` in `transaction` pool mode; `api` and `worker` connect through it, `migrate` deliberately still goes direct (DDL and advisory locks do not survive transaction pooling). Sizing arithmetic documented in the service definition.                                                                                                                                                                                                                                                                                                                             | PgBouncer 1.23.1 run against the live Postgres: Prisma model reads, an interactive transaction, `ST_DWithin` raw SQL and 60 concurrent multiplexed queries all pass, 6/6. A wrong listen port was caught by that run and fixed.              |
| **P-4**       | The global error handler now maps datastore-unreachable errors — `ECONNREFUSED`, `ENOTFOUND`, `ETIMEDOUT`, `EHOSTUNREACH`, `ENETUNREACH`, and Prisma `P1001`/`P1002`/`P1008`/`P1017` — to `503 SERVICE_UNAVAILABLE`, walking the `cause` chain because Prisma wraps the driver error. Matches the Redis path.                                                                                                                                                                                                                                                                                      | `core/errors/error-handler.ts`.                                                                                                                                                                                                              |
| **P-5**       | `auth:epoch:{userId}` carries a TTL tracking `JWT_REFRESH_TTL_SECONDS`, renewed on read so active users keep their epoch and only dormant keys are reaped — by which point the refresh token has expired and an epoch of 0 is correct. `GET`+`EXPIRE` ship as one round trip.                                                                                                                                                                                                                                                                                                                      | `core/cache/stores/EpochStore.ts`.                                                                                                                                                                                                           |
| **P-7**       | `RIDE_DISPATCH_TIMEOUT_SEC` default changed from 30 to 10, matching the stated business rule. `.env.example` updated.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `config/ride/ride.config.ts`.                                                                                                                                                                                                                |
| **F1-01**     | The default role granted during account creation now carries `initialGrant: true` and the epoch consumer skips it. The audit record is kept; the bump is not. Bumping exists to end sessions that predate a privilege change, and a just-created account has none. `grantRole` — a real privilege change — still bumps.                                                                                                                                                                                                                                                                            | `auth.service.ts`, `epoch-invalidation.consumer.ts`.                                                                                                                                                                                         |
| **PLAT-3**    | Import cycle broken at both edges: `pricing/utils/distance.util.ts` deep-imports `haversineKm` instead of pulling the whole `@modules/location` barrel, and `MapProviderService` no longer imports `@core/di` for a service-locator fallback the container already injects.                                                                                                                                                                                                                                                                                                                        | Unit suite **900/909 → 1028/1028**. The 9 files were not merely failing — 119 tests were never running.                                                                                                                                      |
| **P-1**       | The accepted quote is now a ceiling. `pricingConfig.maxFareIncreaseOverQuotePct` (`RIDE_MAX_FARE_INCREASE_OVER_QUOTE_PCT`, default 10) bounds the final fare; the ceiling is solved for in the discount lane so `netFare`, tax and platform fee still reconcile to the paisa instead of the total being clamped and the components no longer summing. `driverEarning` derives from pre-discount `rideRevenue`, so **the platform absorbs the cap, not the driver** — the platform made the promise and traffic is not the driver's fault. A ride with no recorded quote bills uncapped, as before. | New `tests/unit/pricing/fare-ceiling.test.ts`, 8/8: caps land exactly on the ceiling under both percentage and flat platform fees, the FR-006 identity holds, the driver's earning is unchanged, and no ceiling can drive a bill below zero. |

### Corrected

**P-6 was wrong.** It recorded `nearbyDriverEtaStatus: 'matrix_unavailable'` on every quote as a defect. `getDistanceMatrix` is fully implemented on all three providers and makes a real HTTP call; `unavailable` is the correct answer when no live credential is configured, which was the case throughout the audit. There is no product defect here.

It did expose a real asymmetry: routing had an offline stand-in under test and the matrix did not, so the driver-ETA path was untestable — the same "silently unreachable" class as Q-2. `offlineMatrixResult` now covers it under `test` only. Production behaviour is unchanged: a provider error still returns `unavailable`.

### Still open

- **PLAT-1** (no payment gateway) and **PLAT-2** (no push provider) remain **P0**. Both need vendor credentials and verification against the real API. Writing unverifiable integration code and lifting the boot guards would reproduce exactly the defect class this audit documented — code reporting success without doing the work — so the guards stand until the integrations can be proven.
- **60k / 30k capacity remains NOT PROVEN.** PgBouncer removes the connection-budget ceiling that made the target arithmetically unreachable, and it is verified to work; it is not itself a capacity measurement. That still needs a staging environment: two or more instances behind a load balancer, managed Redis, and a load generator outside the box. `DB_POOL_MAX` stays at its default of 10 deliberately — raising it belongs with that measurement, not with a guess here.
- P-8, P-9, P-10, Q-5, Q-6, Q-7, F1-02…F1-05, F2-01…F2-03 are unchanged.
