...
| A4  | Payment: the customer pays the **driver directly** — **cash, UPI (the rider's own app: Google Pay / PhonePe / Paytm / any) or card**. Zaroorat does **not** collect the ride fare. The platform's own gateways (**Razorpay / Stripe**; `mock` for development) take **driver** money only: subscription payments and commission-wallet recharges. There is no customer wallet. | Payments module scope, reconciliation                |
