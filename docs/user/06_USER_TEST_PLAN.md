...
| **USER-INV-8** | The erasure job acts only on a recorded, due, **uncancelled** request. A restore cancels it in the restore transaction; a due request whose account has an open obligation is left `PENDING` and nothing is touched.                     | unit + integration       |
