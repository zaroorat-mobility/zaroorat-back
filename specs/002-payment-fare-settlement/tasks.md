...
- [x] T091 Walk the quickstart scenarios — [quickstart.md](./quickstart.md) — **MET.** All 11 scenarios walked against a live `npm run dev` + `npm run worker:dev` pointed at `zaroorat_test`: **97 checks, 0 failures** (phase A, flag OFF: 72; phase B, flag ON: 25). Every API assertion went over real HTTP to the running server and every job went onto the real BullMQ queue the running worker consumes — nothing was called in-process, because the wiring is what this task exists to test. Collection was never drained by hand: the scenarios poll until the server's own outbox relay has caught up.

  **What only the live run could show.** BD-5's gating is structural in both directions: with the flag off `POST /rides/:id/payment/confirm-cash` returns 404 and does not appear in the served OpenAPI document at all; after a restart with the flag on, the same route is registered and works. The relay's own timer collects a completed ride with no test-side drain. The worker picks up `payment-collection-sweep`, `payment-receivable-writeoff`, `payment-reconciliation` and `driver-settlement` off the queue and runs them to completion.

  **One real defect found and fixed — `payment.reconciliation.mismatch` was never published.** The event has been declared in `PAYMENT_EVENT_CATALOG` since the module was written, and Scenario 7 requires it, but `ReconciliationJob` only recorded a `wallet_reconciliations` row, incremented a metric and wrote a log line. A divergence between a stored balance and the ledger — the single thing the job exists to detect — therefore notified nothing that could act on it. The job now injects `eventPublisher` and publishes the event for both the customer-wallet and driver-wallet mismatch branches. No integration test caught this because they all assert the `wallet_reconciliations` row, which was written correctly.

  **One quickstart step could not be run as written.** Scenario 3 step 3 says to configure the mock gateway to decline; `MockGatewayProvider` has no decline switch and always succeeds, so a live caller cannot make it fail. The failure ladder was driven instead by a WALLET ride against an empty wallet, which reaches the same code through `InsufficientBalanceError`: same `FAILED` attempt rows, same doubling backoff, same attempt cap, same transition to `UNPAID`. The retry cap, the `willRetry: false` event, `collectionState` never leaking `FAILED`, and `GET /me/debt` were all asserted on that path.

  **Two harness affordances, both recorded rather than hidden.** The walkthrough clears the limiter's own Redis keys between logins and retries once on a 429, because it creates a dozen accounts in two minutes and trips limits no real client would; the limiter itself is running throughout. Ageing rows (`completed_at`, `ride_payments.created_at`) is how the clock is advanced for the backoff ladder, the 90-day write-off and the BD-6 grace period.
